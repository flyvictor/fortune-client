...
const fortuneClient = require('../../lib/fortune-client');
const _ = require('lodash');
const isomorphicClient = require('../../lib/isomorphic-http-client');
const resourcesResolver = require('../../lib/initializer/resources-resolver');
...
module.exports = function (util) {
  describe('Fortune-client in proc', function () {
    ...
    beforeEach(async function () {
      util.client = client = fortuneClient([
        setup.apps.users.fortune,
        setup.apps.bands.fortune,
      ]);

      await client.ready;
      resourceNames = _.pluck(
        _.union.apply(
          _,
          _.map(setup.apps, function (app) {
    ...
    it('requests resource metadata (deprecated)', function () {
      _.pluck(client.resources, 'name')
        .sort()
        .should.be.eql(resourceNames.sort());
    });

    it('requests resource metadata', async function () {
      const resources = await client.getResources();

      _.pluck(resources, 'name').sort().should.be.eql(resourceNames.sort());
    });
    ...
    it('allows getting a collection of resources', function (done) {
      client
        .getUsers()
        .then(function (data) {
          data.users.length.should.be.equal(
            setup.apps.users.resources.users.length,
          );
          done();
        })
        .catch(function (err) {
    ...
    it('should throw if singular destroy is called with null-like id', function () {
      try {
        ...
        e.message.should.equal(
          'Operating on a collection with a singularised action. Use the pluralised form.',
        );
      }
      try {
        client.destroyUser();
    ...
    it('allows replacing a document', function (done) {
      ...
      client
        ...
        .then(function (data) {
          user = data.users[0];
          return client.replaceUser(
            user.id,
            _.extend({}, user, { name: '1234' }),
          );
        })
        .then(function (data) {
    ...
    describe('alternative programmatic syntax', function () {
      ...
      it('supports additional parameters', function (done) {
        client
          .updateUsers(ids.users[0], [
            {
              op: 'add',
              path: '/users/0/links/instruments',
              value: ids.instruments[0],
            },
          ])
          .then(function () {
            client
              .getUsers({ name: 'Bob' }, { include: 'instruments' })
              .then(function (reg) {
                client
                  .get('users', { name: 'Bob' }, { include: 'instruments' })
                  .then(function (prog) {
                    reg.should.eql(prog);
                    done();
                  });
      ...
      it('should not duplicate included resources from different paths', function (done) {
        client
          .updateUsers(ids.users[0], [
            {
              op: 'replace',
              path: '/users/0/links/band',
              value: [ids.bands[0]],
            },
            {
              op: 'replace',
              path: '/users/0/links/bababand',
              value: [ids.bands[0]],
            },
          ])
          .then(function () {
            client
              .getUsers(ids.users[0], { include: 'band,bababand' })
              .then(function (res) {
  ...
  describe('Fortune-client remote', function () {
    ...
    it('should try to refresh fortune client and throw handled error from proxy if method does not exists', async function () {
      const spy = sinon.spy(isomorphicClient, 'get');

      try {
        await client.getRandomResource({ name: 'Sweeney' });
        throw new Error('should not reach this line');
      } catch (err) {
        err.message.should.eql(
          'Method getRandomResource is not defined on Fortune client context.',
        );
      } finally {
        spy.callCount.should.eql(2);

        should.exist(
          spy.getCalls().find((call) => {
            return (
              call.args[0].uri ===
              'http://localhost:9782/resources/fortune-client'
            );
          }),
        );
        ...
        should.exist(
          spy.getCalls().find((call) => {
            return (
              call.args[0].uri ===
              'http://localhost:9783/resources/fortune-client'
            );
          }),
        );
    ...
    it('should refresh fortune client and execute new inited method if new resources found', async function () {
      ...
      try {
        ...
        should.exist(
          getResourcesStub.getCalls().find((call) => {
            return (
              call.args[0].uri ===
              'http://localhost:9782/resources/fortune-client'
            );
    ...
    describe('cache', function () {
      it('should get data from cache if available', async function () {
        ...
        await client.ready;
        should.exist(Object.keys(client).find((e) => e === 'getCacheUsers'));
      });

      it('should refresh cache when method not found resources refresh', async function () {
        const cacheSet = sinon.stub();
        client = fortuneClient(['http://localhost:9782'], {
          persistentCache: {
            get: sinon.stub().resolves(
              JSON.stringify([
        ...
        should.exist(Object.keys(client).find((e) => e === 'getCacheUsers'));
        should.not.exist(Object.keys(client).find((e) => e === 'getUsers'));

        await client.getUsers();
        cacheSet.callCount.should.eql(2);

        should.exist(Object.keys(client).find((e) => e === 'getCacheUsers'));
        should.exist(Object.keys(client).find((e) => e === 'getUsers'));
      });

      it('should avoid multiple concurrent get resources', async function () {
        const getResourcesStub = sinon.spy(
          resourcesResolver,
          'fetchResources',
        );
        ...
        try {
          ...
          await client.ready;
          getResourcesStub.callCount.should.equal(0);
          ...
          getResourcesStub.callCount.should.equal(1);
        } finally {
          getResourcesStub.restore();
        }
      });
    ...
    describe('denormalization', function () {
      ...
      it('should denormalize one-to-one refs', function (done) {
        client
          .getUser(ids.users[0], { include: 'lover', denormalize: true })
          .then(function (res) {
      ...
      it('should denormalize one-to-many refs', function (done) {
        client
          .getUser(ids.users[0], { include: 'address', denormalize: true })
          .then(function (res) {
      ...
      it('should denormalize one-to-many > one-to-many refs', function (done) {
        Promise.all(
          _.map(ids.instruments, function (instument, index) {
            return client
              .updateInstrument(instument, [
                {
                  op: 'replace',
                  path: '/users/0/owner',
                  value: ids.users[index],
                },
              ])
              .then(function () {
          ...
          client
            .getInstruments(
              {},
              { include: 'owner,owner.band', denormalize: true },
            )
            .then(function (res) {
              _.each(res.instruments, function (instument) {
                instument.links.owner.should.be.an.Object;
                instument.links.owner.id.should.eql(
                  ids.users[ids.instruments.indexOf(instument.id)],
                );
                instument.links.owner.links.band.should.be.an.Object;
                instument.links.owner.links.band.id.should.eql(
                  ids.bands[ids.instruments.indexOf(instument.id)],
                );
              });
              done();
            });
      ...
      it('should denormalize many-to-one refs', function (done) {
        client
          .getUser(ids.users[0], { include: 'instruments', denormalize: true })
          .then(function (res) {
            res.users[0].links.instruments.length.should.equal(1);
            res.users[0].links.instruments[0].should.be.an.Object;
            res.users[0].links.instruments[0].id.should.equal(
              ids.instruments[0],
            );
            done();
          });
      ...
      it('should denormalize deeply linked documents', function (done) {
        client
          .getAddress(ids.addresses[0], {
            include: 'inhabitants,inhabitants.instruments',
            denormalize: true,
          })
          .then(function (res) {
            res.addresses[0].links.inhabitants[0].should.be.an.Object;
            res
              .addresses[0].links.inhabitants[0].links.instruments[0].should.be.an.Object;
            done();
          });
      });
