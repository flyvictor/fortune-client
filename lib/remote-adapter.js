...
module.exports = function(host) {
  ...
  return {
    ...
    'callAction': function(resource, method, req){
      var deferred = when.defer();
      if(!req.params || !req.params.id || !req.params.key) {
        deferred.reject(Error("[remote-adapter] - callAction: req must contains parameters id & key."));
      }

      var params = Object.assign({
        method: method,
        uri: [url, resource, req.params.id, req.params.key].join('/'),
        json: true,
        body: req.body || null, 
        qs: req.query || null
      }, sharedParams);
