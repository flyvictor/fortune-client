...
module.exports = function (app, resources, actionNames) {
      ...
      const dispatch = function () {
        return app[method](resource, request).then(function (result) {
          ...
          if (result && result.body && result.body.error)
            throw new Error(result.body.detail || result.body.error);
          return result;
        });
      };
    ...
    return {
      get: function (query, options) {
        return wrapCall(
          name,
          resource.route,
          util.toDirectRequest(query, options),
          options,
        );
      },
      ...
      replace: function (id, data, options) {
        if (util.isCollectionQuery(id))
          throw new Error('Collection queries not allowed in replace!');
      ...
      update: function (id, data, options) {
        if (util.isCollectionQuery(id))
          throw new Error('Collection queries not allowed in update!');
        return wrapCall(
          name,
          resource.route,
          util.toDirectRequest(id, options, null, data),
          options,
        );
      },
    }[name];
  ...
  function makePluralAction (name, resource) {
    const action = makeAction(name, resource);

    return function (query) {
      if (util.isSingleDocQuery(query)) {
        console.warn(
          `Single document operations with pluralised actions are deprecated. Use the singularised form. Method = ${
            name
            }, Resource = ${
            resource.route
            }, Query = ${
  ...
  function init () {
    _.each(resources, function (resource) {
      const plural = util.toCapitalisedCamelCase(resource.route),
        singular = util.toCapitalisedCamelCase(resource.name);

      _.each(actionNames, function (name) {
        (crud.simple[name] = crud.simple[name] || {})[resource.route] =
          makeAction(name, resource);
    ...
    _.each(actionNames, function (name) {
      crud[name] = function (route) {
        return makeAction(name, { route: route }).apply(
          crud,
          _.rest(arguments),
        );
      };
