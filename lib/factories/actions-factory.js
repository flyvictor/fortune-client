...
module.exports = function (app, resources) {
  ...
  function wrapCall (method, resource, request, options, isGeneric) {
    ...
    return function (cb) {
      const dispatch = function () {
        return app[isGeneric ? 'callGenericAction' : 'callAction'](
          resource,
          method,
          request,
  ...
  function makeAction (resource, actionName, action) {
    const methodName = action.method ? action.method.toUpperCase() : 'POST';
    const isGeneric = action.isGeneric === true;
    ...
    return function (id, data, options) {
      ...
      let query = null;
      if (data && ['GET', 'DELETE'].indexOf(methodName) !== -1) query = data;

      // set data body if not query and data is defined
      var dataBody = null;
      if (query === null && data !== null) var dataBody = data;

      const request = util.toDirectRequest(
        resourceId || null,
        options,
        null,
        null,
      ...
      if ([null, void 0].indexOf(request.params) !== -1)
        throw new Error(
          '[action-factory] - makeAction: request.params cannot be null or undefined!',
        );
  ...
  function init () {
    _.each(resources, function (resource) {
      const resActions = resource.actions || {};
      _.each(resActions, function (resAction, key) {
        const listKey =
          `call${
          util.toCapitalisedCamelCase(resource.name)
          }${util.toCapitalisedCamelCase(key)}`;
