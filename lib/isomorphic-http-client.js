...
const applicationJsonTypes = ['application/vnd.api+json', 'application/json', 'application/problem+json'];
const hasContentType = function(contentType, types){
  if (!contentType || !types) return null;
  return _.reduce(types, function(acc, val) {
...
exports._send = function(params){
  ...
  return fetch(params.fullUrl, fetchOptions)
    .then(function(res){ 
      let result = res;
      if (res.status !== 204) { // no content. Example: delete doesn't send back any content
        if (hasContentType(res.headers.get("content-type"), applicationJsonTypes)) {
          result = res.json();
        }
      } else result = null;
    ...
    .then(function(result) {
      ...
      const body = result[1]
      if (statusCode >= 400) throw { statusCode, errors: body && body.errors || [] }
