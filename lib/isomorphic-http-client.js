...
function shouldSetUserAgent(headers) {
  return headers['user-agent'] == null && typeof window === undefined;
}
...
exports._send = async function (params) {
  const fetchOptions = {
    method: params.method,
  ...
  const res = await fetch(params.fullUrl, fetchOptions);
  ...
  return { statusCode, body, headers: res.headers };
};
