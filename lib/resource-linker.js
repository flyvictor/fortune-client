...
module.exports = function(router) {
  router.onResponse(function(req,res){
    if(!res.body) {
      return when.resolve();
    } else {
      return fetchExternals(includes(req.options),res.body, req.options.user).catch(function(err) {
        console.trace(err);
      });
    }
  });
  ...
  function fetchAncestor(type, ids, includePath, user) {
    return router.actions["get" + util.toCapitalisedCamelCase(type)](ids, {include: includePath}, user);
  }
   ...
   * @api private
   */
  function fetchExternals(includes, body, user) {
