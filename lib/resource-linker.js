...
module.exports = function(router) {
   ...
   */
  function fields(req) {
    return (req.query || {}).fields && typeof req.query.fields === 'object' && req.query.fields;
  }
  ...
  function fetchAncestor(type, ids, includePath, fields, req) {
    ...
    return router.actions["get" + util.toCapitalisedCamelCase(type)](ids, options);
  }
  ...
  function mergeLinks(source, target, pathArr, ancestorType) {
    var refType = ancestorType + ((pathArr.length > 1) ? "." + _.tail(pathArr).join(".") : "");
  ...
  function mergeAncestorData(target, pathArr, type, source) {
    mergeLinked(source, target, _.tail(pathArr), type);
  ...
  function fetchExternals(request, body) {
    ...
    return Promise.all(_.map(requestsByType, function(requestData, ancestorType) {
      var ids = _.uniq(requestData.ids);
      return fetchAncestor(ancestorType, ids, _.compact(_.map(requestData.includes, function(i) { return _.tail(i).join("."); })).join(","), fields(request), request)
