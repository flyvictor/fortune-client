...
module.exports = function(router) {
  router.onResponse(function(config,res){
    if(!res.body) {
      ...
      return fetchExternals(config.request, res.body).then(function(body){
        return config.autoLink ? autoLink(body): body;
      }).catch(function(err) {
        console.trace(err);
      });
    }
  ...
  function autoLink(body){
    var root = body[rootName(body)];
    _.each(root, function(pRes){
      _.each(pRes.links, function(linkV,linkK){
        if(_.isArray(linkV)){
   ...
   * @api private
   */
  function fetchAncestor(type, ids, includePath, user) {
    return router.actions["get" + util.toCapitalisedCamelCase(type)](ids, {include: includePath, user: user});
  }
  ...
  function fetchExternals(request, body) {
    ...
    return when.all(_.map(requestsByType, function(requestData, ancestorType) {
        ...
        return fetchAncestor(ancestorType, ids, _.compact(_.map(requestData.includes, function(i) { return _.rest(i).join("."); })).join(","), user)
