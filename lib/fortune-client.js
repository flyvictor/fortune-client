    ...
    return when.resolve().then(() => {
      ...
      if(_.isObject(app) && app.resources){
        ...
        return isomorphicClient.get(request).then(function(data){
          ...
          router.registerActions(remoteAdapter(app, options), resources);
          router.registerResources(remoteAdapter(app, options), resources);
      ...
      if (attempt < 10000) {
        return delay(2000).then(() => initApp(app, router, attempt + 1))
      } else {
        throw err
      }
    })
