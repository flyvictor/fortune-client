...
exports.resolveReferencesFilters = function(config){
  ...
  var filter = config.options.query.filter;
  var dropId = false;
  var hash = {};
  if (resourceSchema){
      ...
      if (path === "$or" || path === "$and") {
        if (filter.id) {
          ...
          hash.$and = when.all(hash.$and.concat([
            {id: filter.id},
            keys.all({id: exports.unwrapOrFilter(currentResource, query, config.options.user)})
          ]));
          dropId = true;
        } else {
          hash.id = exports.unwrapOrFilter(currentResource, query, config.options.user);
        }
        ...
        hash[path] = exports.requestSubresource(externalResourceName, query, config.options.user)
          .then(function (ids) {
            return {$in: ids};
          });
      }else{
...
exports.inspectRequest = function(config){
  if (!config.options.query.filter) return when.resolve();
  return exports.resolveReferencesFilters(config).then(function(rectifiedFilter){
    config.options.query.filter = rectifiedFilter;
    return when.resolve();
  });
};
