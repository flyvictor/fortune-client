...
module.exports = function(app, resources,actionNames){
  ...
  function makeFluentAction( resource ){
    var modifiers = ["include", "fields", "sort", "limit", "page", "pageSize"];
    ...
    var fluent = {
      ...
      options: {},
      execute: function(){
        return wrapCall( this.method, resource,
                         toDirectRequest( this.query, this.options),
                         this.options)(router.actionPipeline);
      }
    };
    _.each( actionNames, function( actionName ){

      // Don't know how we want other action types to behave so only create 'get'
      // methods for now
      if( actionName === 'get' ){
        fluent[ actionName ] = function( query ){
          fluent.query = query;
          fluent.method = actionName;
          return fluent;
        }
      }
    });

    // for each modifier type ( sort, limit etc...) add a method to populate the
    // relevant part of the query
    _.each( modifiers, function( modifier ){
      fluent[ modifier ] = function( query ){
        fluent.options[ modifier ] = query;
        return fluent;
      }
    });
  ...
  function init(){
    _.each(resources, function(resource){
      ...
      crud.fluent[ resource.name ] = makeFluentAction( resource.name );
      crud.fluent[ pluralize( resource.name ) ] =
      makeFluentAction( pluralize( resource.name ));
