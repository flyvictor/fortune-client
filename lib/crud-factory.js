...
module.exports = function(app, resources){
  ...
  function normaliseQuery(query){
    if(query instanceof mongoose.Types.ObjectId) return query.toString();

    if(_.isArray(query)){
      return _.map(query, function(item){
        return (item instanceof mongoose.Types.ObjectId) ? item.toString() : item;
      });
    };
