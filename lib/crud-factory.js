...
module.exports = function(app, resources,actionNames){
  ...
  function toDirectRequest(query, options, resourceName, data){
    ...
    if(data){
      if(_.has(data, resourceName)){
        //Using http compatible interface
        request.body = data;
      }else if(resourceName){
        //Initial fortune-client implementation
        data =  _.isArray(data) ? data : [data];
        (request.body = {})[resourceName] = data;
      }else{
        //update operations with no pointer to resource in the body
        data =  _.isArray(data) ? data : [data];
        request.body = data;
      }
