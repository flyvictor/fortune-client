...
const toDirectRequest = function (query, options, resourceName, data) {
  options = options || {};
  //REFACTOR: bit ugly this extend...
  const request = _.extend(
    { query: {}, params: {} },
    _.omit(options, 'linked'),
  );
  ...
  _.each(this.queryKeys, function (keyword) {
    if (!_.isUndefined(request[keyword])) {
      request.query[keyword] = request[keyword];
      delete request[keyword];
    }

    const kArgs = request.query[keyword];
...
const util = (module.exports = {
  ...
  toCapitalisedCamelCase: function (str) {
    return _.map(str.split('-'), function (substr) {
