...
module.exports = function(fortuneClient){
  var self = {
    actions: {},
    _onRequestFns: [],
    _onResponseFns: [],
    registerResources: function(app,resources){
      var crud = crudFactory(app, resources, fortuneClient);
      
      _.each(crud, function(action, name){
        self.actions[name] = function(){
          return action.apply(crud,arguments)(function(request, dispatch){
            var response;
            return self._buildCallbackChain(self._onRequestFns)(request).then(function(){
              return dispatch();
            }).then(function(res){
              response = res;
              return self._buildCallbackChain(self._onResponseFns)(request,response);
            }).then(function(){
