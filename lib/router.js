...
const actionsFactory = require('./factories/actions-factory');
const _ = require('lodash');

const ACTION_NAMES = ['get', 'create', 'replace', 'update', 'destroy'];
...
module.exports = function () {
  ...
  const self = {
    ...
    registerResources: function (app, resources) {
      const crud = crudFactory(app, resources, ACTION_NAMES);
  ...
  function actionPipeline(config, dispatch) {
    ...
    //make truly private
    return buildCallbackChain(onRequestFns)(config)
      .then(function () {
  ...
  _.each(ACTION_NAMES, function (actionName) {
